function SVMModels = crosstrainsvm(cost, kernel, descriptors, labels, FILENAME)
%crosstrainsvm - trains 10 seperate SVMModels, if desired saving them into a file.
%inputs: 
...
for model = 1:10
   ...
   for dataset = 1:10
       ...
       %concatenating the descriptors labels from every sub-dataset except one (which is
   ...
   if(isnumeric(kernel))
    SVMModels(model).value = fitcsvm(traindesc, trainlabel,'ClassNames',[-1,1],'Cost',cost,'KernelFunction','polynomial','PolynomialOrder',kernel);
   else
       SVMModels(model).value = fitcsvm(traindesc, trainlabel,'ClassNames',[-1,1],'Cost',cost,'KernelFunction',kernel);
   end
end
toc

%If a filename is given, the SVMModels are saved to a file named FILENAME.
if(~isempty(FILENAME))
